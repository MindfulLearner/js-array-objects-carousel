...
let chargingImage = document.querySelectorAll('.imgClick');


for (let i = 0; i < chargingImage.length; i++){
    chargingImage[i].src = images[i].img;
    chargingImage[i].addEventListener('click', function(){
        console.log('cliccato numero: ' + i); 
        card[contaImg].classList.add('hidden');
        contaImg = i;
        card[contaImg].classList.remove('hidden');
    });
...
let interruttoreStartStop = false;
console.log('interrutore: ' + interruttoreStartStop);
let startStop = document.querySelector('.start-stop');
startStop.addEventListener('click', function() {
    console.log('swag partito click');
    if (interruttoreStartStop == false) {
        console.log('interrutore true');
        interruttoreStartStop = true;
    } else {
        console.log('interruttore false');
        interruttoreStartStop = false;
    }
    while (interruttoreStartStop == true) {
    threeSecondTimer();
    }
});
...
function threeSecondTimer() {
        let timeFine;
        timeFine = setTimeout(cambiaImgDestra, 3000);
}

function cambiaImgDestra() {
    console.log('entrato funzione cambio destra');
